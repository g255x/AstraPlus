...
import dev.Astra.mod.modules.impl.render.BreakESP;
...
implements Wrapper {
    ...
    public void onTick(ClientTickEvent event) {
        ...
        if (AntiCheat.INSTANCE.detectDouble.getValue()) {
            Iterator<Object> iterator = ((ConcurrentHashMap.KeySetView) Astra.BREAK.doubleMap.keySet()).iterator();
            while (iterator.hasNext()) {
                int i = (Integer)iterator.next();
                BreakData breakData = Astra.BREAK.doubleMap.get(i);
    ...
    public void onPacket(PacketEvent.Receive event) {
        ...
        if (packet instanceof BlockBreakingProgressS2CPacket) {
            ...
            Astra.EVENT_BUS.post(BlockBreakingProgressEvent.get(packet2.getPos(), packet2.getEntityId(), packet2.getProgress()));
