...
implements Wrapper {
    ...
    public boolean inFov(Vec3d directionVec, float fov) {
        ...
        return this.inFov(angle[0], angle[1], fov, RotationManager.mc.player.getYaw(), RotationManager.mc.player.getPitch());
    }
    ...
    public boolean inFov(float yaw, float pitch, float fov, float currentYaw, float currentPitch) {
        float pitchDifferent;
        float yawDifferent = MathHelper.angleBetween((float)yaw, (float)currentYaw);
        return yawDifferent * yawDifferent + (pitchDifferent = Math.abs(pitch - currentPitch)) * pitchDifferent <= fov * fov;
    }
    ...
    public void update(SendMovementPacketsEvent event) {
        if (AntiCheat.INSTANCE.movementSync() && !BaritoneUtil.isActive()) {
            ...
            Astra.EVENT_BUS.post(updateRotateEvent);
    ...
    public void onLastRotation(UpdateRotateEvent event) {
        ...
        Astra.EVENT_BUS.post(rotationEvent);
    ...
    public void setLastYaw(float lastYaw) {
        ...
        if (AntiCheat.INSTANCE.forceSync.getValue() && Astra.SERVER.playerNull.passedS(0.15)) {
