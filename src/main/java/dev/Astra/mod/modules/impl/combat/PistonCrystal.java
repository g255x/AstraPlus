...
public class PistonCrystal extends Module {
    ...
    private final SliderSetting placeRange = this.add(new SliderSetting("PlaceRange", 4.0, 1.0, 8.0, 0.1).setSuffix("m"));
    private final SliderSetting range = this.add(new SliderSetting("Range", 6.0, 1.0, 8.0, 0.1).setSuffix("m"));
    private final SliderSetting breakRange = this.add(new SliderSetting("BreakRange", 3.0, 1.0, 8.0, 0.1).setSuffix("m"));
    private final BooleanSetting pistonPacket = this.add(new BooleanSetting("PistonPacket", true));
    private final BooleanSetting usingPause = this.add(new BooleanSetting("UsingPause", false));  // 原 noEating，反转默认值
    private final BooleanSetting usingBreak = this.add(new BooleanSetting("UsingBreak", true));   // 原 eatingBreak，反转默认值
    ...
    private final BooleanSetting onlyStatic = this.add(new BooleanSetting("MovingPause", false));
    private final BooleanSetting inventory = this.add(new BooleanSetting("InventorySwap", true));
    private final BooleanSetting rotate = this.add(new BooleanSetting("Rotate", true));
    private final SliderSetting updateDelay = this.add(new SliderSetting("PlaceDelay", 150.0, 0, 500).setSuffix("ms"));
    private final SliderSetting posUpdateDelay = this.add(new SliderSetting("PosUpdateDelay", 200.0, 0, 1000).setSuffix("ms"));
    private final SliderSetting stageSetting = this.add(new SliderSetting("Stage", 2.0, 1, 10));
    ...
    private final SliderSetting powerStage = this.add(new SliderSetting("PowerStage", 2, 1, 10));
    private final SliderSetting powerMaxStage = this.add(new SliderSetting("PowerMaxStage", 2, 1, 10));
    private final SliderSetting crystalStage = this.add(new SliderSetting("CrystalStage", 2, 1, 10));
    private final SliderSetting crystalMaxStage = this.add(new SliderSetting("CrystalMaxStage", 2, 1, 10));
    private final SliderSetting minDamage = this.add(new SliderSetting("MinDamage", 6.0, 0.0, 20.0, 0.5).setSuffix("hp"));
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (this.usingPause.getValue() && PistonCrystal.mc.player.isUsingItem()) {
            return;
        }
        ...
        double breakRangeSq = this.breakRange.getValue() * this.breakRange.getValue();
        if (!PistonCrystal.mc.player.isUsingItem() || this.usingBreak.getValue()) {
            if (this.checkCrystal(pos.up(0))) {
                BlockPos crystalPos = pos.up(0);
                if (PistonCrystal.mc.player.squaredDistanceTo(crystalPos.getX() + 0.5, crystalPos.getY() + 0.5, crystalPos.getZ() + 0.5) <= breakRangeSq) {
                    CombatUtil.attackCrystal(crystalPos, this.rotate.getValue(), true);
                }
            }
            if (this.checkCrystal(pos.up(1))) {
                BlockPos crystalPos = pos.up(1);
                if (PistonCrystal.mc.player.squaredDistanceTo(crystalPos.getX() + 0.5, crystalPos.getY() + 0.5, crystalPos.getZ() + 0.5) <= breakRangeSq) {
                    CombatUtil.attackCrystal(crystalPos, this.rotate.getValue(), true);
                }
            }
            if (this.checkCrystal(pos.up(2))) {
                BlockPos crystalPos = pos.up(2);
                if (PistonCrystal.mc.player.squaredDistanceTo(crystalPos.getX() + 0.5, crystalPos.getY() + 0.5, crystalPos.getZ() + 0.5) <= breakRangeSq) {
                    CombatUtil.attackCrystal(crystalPos, this.rotate.getValue(), true);
                }
            }
    ...
    private boolean checkCrystal(BlockPos pos) {
        float threshold = (float) this.minDamage.getValue();
        for (Entity entity : BlockUtil.getEntities(new Box(pos))) {
            float damage;
            if (!(entity instanceof EndCrystalEntity) || !((damage = AutoCrystal.INSTANCE.calculateDamage(entity.getPos(), this.target, this.target)) > threshold)) continue;
            return true;
        }
        return false;
    }
    ...
    private void getPos(BlockPos pos, Direction facing, BlockPos oPos) {
        ...
        if (!PistonCrystal.mc.world.isAir(pos.offset(facing, -1)) || this.getBlock(pos.offset(facing.getOpposite())) == Blocks.MOVING_PISTON && !this.checkCrystal2(pos.offset(facing.getOpposite()))) {
            return;
        }
    ...
    private boolean isPiston(BlockPos pos, Direction facing) {
        ...
        return PistonCrystal.mc.world.isAir(pos.offset(facing, -1)) || this.getBlock(pos.offset(facing.getOpposite())) == Blocks.MOVING_PISTON;
    }

    private void doRedStone(BlockPos pos, Direction facing, BlockPos crystalPos) {
        if (!PistonCrystal.mc.world.isAir(pos.offset(facing, -1)) && this.getBlock(pos.offset(facing.getOpposite())) != Blocks.MOVING_PISTON) {
            return;
        }
        for (Direction i : Direction.values()) {
