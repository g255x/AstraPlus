...
public class Burrow extends Module {
    ...
    private final EnumSetting<LagBackMode> lagMode = this.add(new EnumSetting<LagBackMode>("LagMode", LagBackMode.Normal));
    private final EnumSetting<LagBackMode> aboveLagMode = this.add(new EnumSetting<LagBackMode>("MoveLagMode", LagBackMode.Normal));
    private final List<BlockPos> placePos = new ArrayList<BlockPos>();
    private final Timer timer = new Timer();
    private final Timer webTimer = new Timer();
    private final BooleanSetting disable = this.add(new BooleanSetting("Disable", false));
    private final BooleanSetting jumpDisable = this.add(new BooleanSetting("JumpDisable", false, () -> !this.disable.getValue()));
    private final SliderSetting delay = this.add(new SliderSetting("Delay", 400, 0, 1000, () -> !this.disable.getValue()).setSuffix("ms"));
    private final SliderSetting webTime = this.add(new SliderSetting("WebTime", 50, 0, 500).setSuffix("ms"));
    ...
    private final BooleanSetting single = this.add(new BooleanSetting("SingleBlock", true));
    private final BooleanSetting detectMine = this.add(new BooleanSetting("DetectMining", true));
    ...
    private final BooleanSetting sound = this.add(new BooleanSetting("Sound", false));
    private final SliderSetting blocksPer = this.add(new SliderSetting("BlocksPer", 2.0, 1.0, 4.0, 1.0));
    private final BooleanSetting breakCrystal = this.add(new BooleanSetting("Break", true));
    private final BooleanSetting wait = this.add(new BooleanSetting("Wait", false, this.disable::getValue));
    private final BooleanSetting fakeMove = this.add(new BooleanSetting("FakeMove", false).setParent());
    private final BooleanSetting center = this.add(new BooleanSetting("AllowCenter", false, this.fakeMove::isOpen));
    private final SliderSetting preCorrect = this.add(new SliderSetting("PreCorrect", 0.25, 0.0, 1.0, 0.001, this.fakeMove::isOpen).setSuffix("m"));
    private final SliderSetting moveDis = this.add(new SliderSetting("MoveDis", 0.25, 0.0, 1.0, 0.001, this.fakeMove::isOpen).setSuffix("m"));
    private final SliderSetting moveDis2 = this.add(new SliderSetting("MoveDis2", 0.25, 0.0, 1.0, 0.001, this.fakeMove::isOpen).setSuffix("m"));
    private final SliderSetting moveCorrect2 = this.add(new SliderSetting("Correct", 0.25, 0.0, 1.0, 0.001, this.fakeMove::isOpen).setSuffix("m"));
    private final SliderSetting yOffset = this.add(new SliderSetting("YOffset", 0.01, 0.0, 1.0, 0.001, this.fakeMove::isOpen).setSuffix("m"));
    private final BooleanSetting inventory = this.add(new BooleanSetting("InventorySwap", true));
    private final SliderSetting smartX = this.add(new SliderSetting("SmartXZ", 2.0, 0.0, 10.0, 0.1, () -> this.lagMode.getValue() == LagBackMode.Smart || this.aboveLagMode.getValue() == LagBackMode.Smart).setSuffix("m"));
    private final SliderSetting smartUp = this.add(new SliderSetting("SmartUp", 1.0, 0.0, 10.0, 0.1, () -> this.lagMode.getValue() == LagBackMode.Smart || this.aboveLagMode.getValue() == LagBackMode.Smart).setSuffix("m"));
    private final SliderSetting smartDown = this.add(new SliderSetting("SmartDown", 2.0, 0.0, 10.0, 0.1, () -> this.lagMode.getValue() == LagBackMode.Smart || this.aboveLagMode.getValue() == LagBackMode.Smart).setSuffix("m"));
    private final SliderSetting smartDistance = this.add(new SliderSetting("SmartDistance", 2.0, 0.0, 10.0, 0.1, () -> this.lagMode.getValue() == LagBackMode.Smart || this.aboveLagMode.getValue() == LagBackMode.Smart).setSuffix("m"));
    ...
    public void onUpdate(ClientTickEvent event) {
        ...
        if (event.isPost()) {
            ...
            if (Astra.PLAYER.isInWeb((PlayerEntity)Burrow.mc.player)) {
                this.webTimer.reset();
                return;
            }
            ...
            if (this.rotate.getValue() == RotateMode.Bypass) {
                if (above) {
                    ...
                    Astra.ROTATION.snapAt(angle[0], angle[1]);
                } else {
    ...
    private boolean canPlace(BlockPos pos) {
        ...
        if (this.detectMine.getValue() && Astra.BREAK.isMining(pos)) {
            return false;
        }
