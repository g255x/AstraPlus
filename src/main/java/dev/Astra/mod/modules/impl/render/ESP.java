...
import dev.Astra.api.events.impl.EntitySpawnedEvent;
...
public class ESP extends Module {
    ...
    private final ColorSetting hHealth = this.add(new ColorSetting("HighHealth", new Color(-16711936, true), () -> this.mode.getValue() == RenderMode.TwoD));
    private final ColorSetting mHealth = this.add(new ColorSetting("MidHealth", new Color(-256, true), () -> this.mode.getValue() == RenderMode.TwoD));
    private final ColorSetting lHealth = this.add(new ColorSetting("LowHealth", new Color(-65536, true), () -> this.mode.getValue() == RenderMode.TwoD));
    ...
    private final ColorSetting armorDuraColor = this.add(new ColorSetting("ArmorDuraColor", new Color(-13631744, true), () -> this.mode.getValue() == RenderMode.TwoD && this.twoDArmorDura.getValue()));
    ...
    private final ColorSetting endPortalFill = this.add(new ColorSetting("EndPortalFill", new Color(-1858405124, true), () -> this.mode.getValue() != RenderMode.TwoD && this.target.isOpen()).injectBoolean(false));
    private final ColorSetting endPortalOutline = this.add(new ColorSetting("EndPortalOutline", new Color(1694495617, true), () -> this.mode.getValue() != RenderMode.TwoD && this.target.isOpen()).injectBoolean(false));
    private final ColorSetting itemFill = this.add(new ColorSetting("ItemFill", new Color(931429313, true), this.target::isOpen).injectBoolean(true));
    private final ColorSetting itemOutline = this.add(new ColorSetting("ItemOutline", new Color(-8094783, true), this.target::isOpen).injectBoolean(true));
    private final ColorSetting playerFill = this.add(new ColorSetting("PlayerFill", new Color(1694498815, true), this.target::isOpen).injectBoolean(true));
    private final ColorSetting playerOutline = this.add(new ColorSetting("PlayerOutline", new Color(1694495617, true), this.target::isOpen).injectBoolean(true));
    private final ColorSetting chestFill = this.add(new ColorSetting("ChestFill", new Color(1862060803, true), () -> this.mode.getValue() != RenderMode.TwoD && this.target.isOpen()).injectBoolean(false));
    private final ColorSetting chestOutline = this.add(new ColorSetting("ChestOutline", new Color(1694495617, true), () -> this.mode.getValue() != RenderMode.TwoD && this.target.isOpen()).injectBoolean(false));
    private final ColorSetting enderChestFill = this.add(new ColorSetting("EnderChestFill", new Color(965346303, true), () -> this.mode.getValue() != RenderMode.TwoD && this.target.isOpen()).injectBoolean(false));
    private final ColorSetting enderChestOutline = this.add(new ColorSetting("EnderChestOutline", new Color(1694495617, true), () -> this.mode.getValue() != RenderMode.TwoD && this.target.isOpen()).injectBoolean(false));
    private final ColorSetting shulkerBoxFill = this.add(new ColorSetting("ShulkerBoxFill", new Color(-2080414740, true), () -> this.mode.getValue() != RenderMode.TwoD && this.target.isOpen()).injectBoolean(false));
    private final ColorSetting shulkerBoxOutline = this.add(new ColorSetting("ShulkerBoxOutline", new Color(1694495617, true), () -> this.mode.getValue() != RenderMode.TwoD && this.target.isOpen()).injectBoolean(false));
    ...
    private final ColorSetting text = this.add(new ColorSetting("Text", new Color(-1, true), this.item::isOpen));
    ...
    public void onRender3D(MatrixStack matrixStack) {
        if (this.mode.getValue() == RenderMode.TwoD) return;
    ...
    private void draw2DItemText(DrawContext drawContext, ItemEntity itemEntity, float tickDelta) {
        double[] box = this.getEntity2DBox(itemEntity, tickDelta);
        if (box == null) return;
        int itemCount = itemEntity.getStack().getCount();
        String s = this.count.getValue() && itemCount > 1 ? " x" + itemCount : "";
        String name = (this.customName.getValue() ? itemEntity.getStack().getName() : itemEntity.getStack().getItem().getName()).getString();
