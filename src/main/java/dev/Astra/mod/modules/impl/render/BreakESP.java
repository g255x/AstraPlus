...
public class BreakESP
    ...
    private final BooleanSetting name = this.add(new BooleanSetting("Name", true));
    private final BooleanSetting status = this.add(new BooleanSetting("Status", true));
    ...
    private Color getFillColor(PlayerEntity player) {
        return Astra.FRIEND.isFriend(player) ? this.fillFriend.getValue() : this.fill.getValue();
    }

    private Color getBoxColor(PlayerEntity player) {
        return Astra.FRIEND.isFriend(player) ? this.boxFriend.getValue() : this.box.getValue();
    }
    ...
    public void onRender3D(MatrixStack matrixStack) {
        for (BreakManager.BreakData breakData : Astra.BREAK.breakMap.values()) {
            ...
            // 挖掘者名字（受 Name 开关控制，固定偏移 0.15）
            if (this.name.getValue()) {
                Render3DUtil.drawText3D(player.getName().getString(), breakData.pos.toCenterPos().add(0.0, 0.15, 0.0), -1);
            }
            // 状态文字（受 Status 开关控制，完全使用原始API）
        ...
        if (this.second.getValue()) {
            Iterator<BreakManager.BreakData> iterator = ((ConcurrentHashMap.KeySetView) Astra.BREAK.doubleMap.keySet()).iterator();
            while (iterator.hasNext()) {
                int i = (Integer)((Object)iterator.next());
                BreakManager.BreakData breakData = Astra.BREAK.doubleMap.get(i);
                if (breakData == null || breakData.getEntity() == null || BreakESP.mc.world.isAir(breakData.pos)) {
                    Astra.BREAK.doubleMap.remove(i);
                    continue;
                }
                BreakManager.BreakData singleBreakData = Astra.BREAK.breakMap.get(i);
