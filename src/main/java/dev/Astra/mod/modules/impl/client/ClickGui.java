...
public class ClickGui extends Module {
    ...
    public void updateFadeAnimation() {
        if (this.fade.getValue()) {
            this.alphaValue = this.animation.get(this.isOn() ? 1.0 : 0.0, this.length.getValueInt(), this.easing.getValue());
        } else {
            this.alphaValue = 1.0;
        }
    }
    ...
    public class FadeOut {
        ...
        public void onRender2D(Render2DEvent event) {
            boolean isScreenOpen = Wrapper.mc.currentScreen instanceof ClickGuiScreen;
            if (ClickGui.this.fade.getValue()) {
                if (ClickGui.this.alphaValue > 0.0 && !isScreenOpen) {
