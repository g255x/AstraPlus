...
public class Offhand extends Module {
    ...
    private final BooleanSetting switchPickaxe = this.add(new BooleanSetting("SwitchPickaxe", false, this.gapSwitch::isOpen));
    ...
    private int previousPickaxeSlot = -1;
    private boolean pickaxeGapActive = false;
    ...
    public void onTick(ClientTickEvent event) {
        ...
        // 2. 恢复逻辑：如???autoRestore 开启且危险解除，则切回原槽???
        ...
        if (this.pickaxeGapActive && !Offhand.mc.options.useKey.isPressed()) {
            InventoryUtil.switchToSlot(this.previousPickaxeSlot);
            this.pickaxeGapActive = false;
            this.previousPickaxeSlot = -1;
            this.timer.reset();
            return;
        }
        ...
        if (this.gapSwitch.getValue() && Offhand.mc.options.useKey.isPressed()) {
            ...
            if (this.switchPickaxe.getValue() && mainItem instanceof PickaxeItem) {
                int egapSlot = InventoryUtil.findItem(Items.ENCHANTED_GOLDEN_APPLE);
                if (egapSlot != -1) {
                    if (!this.pickaxeGapActive) {
                        this.previousPickaxeSlot = Offhand.mc.player.getInventory().selectedSlot;
                        this.pickaxeGapActive = true;
                    }
                    InventoryUtil.switchToSlot(egapSlot);
                    this.timer.reset();
                    return;
                }
