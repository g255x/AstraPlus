...
public class AutoTrap extends Module {
    ...
    private final EnumSetting<TargetMode> targetMod = this.add(new EnumSetting<TargetMode>("TargetMode", TargetMode.Multi));
    private final EnumSetting<Mode> headMode = this.add(new EnumSetting<Mode>("BlockForHead", Mode.Obsidian));
    final ArrayList<BlockPos> trapList = new ArrayList();
    final ArrayList<BlockPos> placeList = new ArrayList();
    private final Timer timer = new Timer();
    private final SliderSetting placeRange = this.add(new SliderSetting("PlaceRange", 4.5, 1.0, 6.0, 0.1).setSuffix("m"));
    private final SliderSetting blocksPer = this.add(new SliderSetting("BlocksPer", 4, 1, 8));
    public final SliderSetting predictTicks = this.add(new SliderSetting("PredictTicks", 10.0, 0.0, 50.0, 1.0).setSuffix("ticks"));
    ...
    private final BooleanSetting checkElytra = this.add(new BooleanSetting("CheckElytra", true));
    private final BooleanSetting extend = this.add(new BooleanSetting("Extend", false));
    ...
    private final BooleanSetting headExtend = this.add(new BooleanSetting("HeadExtend", false));
    private final BooleanSetting chestUp = this.add(new BooleanSetting("ChestUp", false));
    ...
    private final BooleanSetting ignoreCrawling = this.add(new BooleanSetting("IgnoreCrawling", true, this.chest::getValue));
    private final BooleanSetting legs = this.add(new BooleanSetting("Legs", false));
    private final BooleanSetting legAnchor = this.add(new BooleanSetting("LegAnchor", false));
    ...
    private final BooleanSetting selfGround = this.add(new BooleanSetting("SelfGround", false));
    ...
    private void trapTarget(PlayerEntity target) {
        if (this.onlyHole.getValue() && !Astra.HOLE.isHole(EntityUtil.getEntityPos((Entity)target))) {
            return;
        }
    ...
    private void doTrap(PlayerEntity player, BlockPos pos) {
        ...
        if (this.antiStep.getValue() && (Astra.BREAK.isMining(pos.up(headOffset)) || !this.onlyBreak.getValue())) {
        ...
        if (this.chestUp.getValue()) {
            for (Direction i : Direction.values()) {
                ...
                if (this.onlyBreaking.getValue() && !Astra.BREAK.isMining(pos.up(headOffset))) continue;
    ...
    public BlockPos getHelper(BlockPos pos) {
        ...
        for (Direction i : Direction.values()) {
            if (this.checkMine.getValue() && Astra.BREAK.isMining(pos.offset(i)) || !BlockUtil.isStrictDirection(pos.offset(i), i.getOpposite()) || !BlockUtil.canPlace(pos.offset(i), this.placeRange.getValue(), this.breakCrystal.getValue())) continue;
            return pos.offset(i);
        }
        return null;
    }
    ...
    public BlockPos getHelper(BlockPos pos, Direction ignore) {
        ...
        for (Direction i : Direction.values()) {
            if (i == ignore || this.checkMine.getValue() && Astra.BREAK.isMining(pos.offset(i)) || !BlockUtil.isStrictDirection(pos.offset(i), i.getOpposite()) || !BlockUtil.canPlace(pos.offset(i), this.placeRange.getValue(), this.breakCrystal.getValue())) continue;
            return pos.offset(i);
        }
        return null;
    }
    ...
    private boolean checkEntity(BlockPos pos) {
        ...
        for (Entity entity : Astra.THREAD.getPlayers()) {
