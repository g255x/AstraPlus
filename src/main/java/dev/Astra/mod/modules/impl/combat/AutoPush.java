...
public class AutoPush extends Module {
    ...
    private final BooleanSetting mine = this.add(new BooleanSetting("Mine", false));
    private final BooleanSetting allowWeb = this.add(new BooleanSetting("AllowWeb", true));
    private final SliderSetting updateDelay = this.add(new SliderSetting("Delay", 200, 0, 1000).setSuffix("ms"));
    ...
    private final SliderSetting range = this.add(new SliderSetting("Range", 6.0, 0.0, 6.0, 0.1).setSuffix("m"));
    private final SliderSetting placeRange = this.add(new SliderSetting("PlaceRange", 4.5, 0.0, 6.0, 0.1).setSuffix("m"));
    private final SliderSetting surroundCheck = this.add(new SliderSetting("SurroundCheck", 4.0, 0.0, 4.0, 0.1));
    private final BooleanSetting inventory = this.add(new BooleanSetting("InventorySwap", true));
    private final Timer timer = new Timer();
    private int swapSlot = -1;
    private boolean swapBypass = false;
    private int originalSlot = -1;
    private int swapSelectedSlot = -1;
    ...
    public static void pistonFacing(Direction i) {
        if (i == Direction.EAST) {
            Astra.ROTATION.snapAt(-90.0f, 5.0f);
        } else if (i == Direction.WEST) {
            Astra.ROTATION.snapAt(90.0f, 5.0f);
        } else if (i == Direction.NORTH) {
            Astra.ROTATION.snapAt(180.0f, 5.0f);
        } else if (i == Direction.SOUTH) {
            Astra.ROTATION.snapAt(0.0f, 5.0f);
        }
    }
    ...
    public void onDisable() {
        if (mc.player != null && this.inventory.getValue() && this.swapSlot != -1) {
            if (!this.swapBypass && EntityUtil.inInventory()) {
                int reverseSlot = this.swapSelectedSlot != -1 ? this.swapSelectedSlot : mc.player.getInventory().selectedSlot;
                mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, this.swapSlot, reverseSlot, SlotActionType.SWAP, mc.player);
                mc.player.getInventory().updateItems();
            }
            if (this.originalSlot != -1) {
                mc.player.getInventory().selectedSlot = this.originalSlot;
                mc.getNetworkHandler().sendPacket(new UpdateSelectedSlotC2SPacket(this.originalSlot));
            }
        }
        this.swapSlot = -1;
        this.swapBypass = false;
        ...
        if (this.isTrueFacing(piston, direction) && this.facingCheck(piston) && BlockUtil.clientCanPlace(piston, false)) {
            ...
            if (BlockUtil.getPlaceSide(piston, this.placeRange.getValue()) != null) {
                ...
                if (canPower) {
                    ...
                    if (side != null) {
                        if (this.rotate.getValue()) {
                            Astra.ROTATION.lookAt(piston.offset(side), side.getOpposite());
                        }
                ...
                if (powerFacing != null) {
                    ...
                    if (side2 != null) {
                        if (this.rotate.getValue()) {
                            Astra.ROTATION.lookAt(piston.offset(side2), side2.getOpposite());
                        }
                        if (this.yawDeceive.getValue()) {
                            AutoPush.pistonFacing(direction.getOpposite());
                        }
    ...
    private void doSwap(int slot) {
        if (this.inventory.getValue()) {
            if (this.swapSlot == -1) {
                ...
                this.swapSelectedSlot = AutoPush.mc.player.getInventory().selectedSlot;
            }
    ...
    private Boolean canPush(PlayerEntity player) {
        ...
        if (!this.allowWeb.getValue() && Astra.PLAYER.isInWeb(player)) {
            return false;
        }
        ...
        return (double)progress > this.surroundCheck.getValue() - 1.0 || Astra.HOLE.isHard(new BlockPosX(player.getX(), player.getY() + 0.5, player.getZ()));
    }
