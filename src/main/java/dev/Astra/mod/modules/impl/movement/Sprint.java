...
extends Module {
    ...
    private boolean shouldSprint() {
        if (!(Sprint.mc.player.getHungerManager().getFoodLevel() <= 6 && !Sprint.mc.player.isCreative() || !MovementUtil.isMoving() || this.pause || Sprint.mc.player.isSneaking() && this.sneakingPause.getValue() || Astra.PLAYER.isInWeb((PlayerEntity)Sprint.mc.player) && this.inWebPause.getValue() || mc.player.isUsingItem() && this.usingPause.getValue() || Sprint.mc.player.isRiding() || Sprint.mc.player.hasStatusEffect(StatusEffects.BLINDNESS) && this.blindnessPause.getValue())) {
            switch (this.mode.getValue().ordinal()) {
                case 1: {
                    if (AntiCheat.INSTANCE.movementSync()) {
                        return Sprint.mc.player.input.movementForward > 0.0f;
                    }
                    return HoleSnap.INSTANCE.isOn() || Sprint.mc.options.forwardKey.isPressed() && MathHelper.angleBetween((float)Sprint.mc.player.getYaw(), (float) Astra.ROTATION.rotationYaw) < 40.0f;
                }
                case 3: {
                    if (AntiCheat.INSTANCE.movementSync()) {
                        return Sprint.mc.player.input.movementForward > 0.0f;
                    }
                    return HoleSnap.INSTANCE.isOn() || MathHelper.angleBetween((float)Sprint.getSprintYaw(Sprint.mc.player.getYaw()), (float) Astra.ROTATION.rotationYaw) < 40.0f;
                }
                case 2: {
                    return true;
                }
    ...
    public void rotate(UpdateRotateEvent event) {
        ...
        if (!((Sprint.mc.player.getHungerManager().getFoodLevel() <= 6 && !Sprint.mc.player.isCreative() || !MovementUtil.isMoving() || Freecam.INSTANCE.isOn() || Sprint.mc.player.isFallFlying() || Astra.PLAYER.isInWeb((PlayerEntity)Sprint.mc.player) && this.inWebPause.getValue() || Sprint.mc.player.isSneaking() && this.sneakingPause.getValue() || Sprint.mc.player.isRiding() || mc.player.isUsingItem() && this.usingPause.getValue() || Sprint.mc.player.isInFluid() || !Freecam.INSTANCE.isOff() || Sprint.mc.player.hasStatusEffect(StatusEffects.BLINDNESS)) && this.blindnessPause.getValue() || !this.mode.is(Mode.Rotation) || event.isModified())) {
