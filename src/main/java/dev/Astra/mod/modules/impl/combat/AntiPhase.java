...
import dev.Astra.api.events.impl.BlockBreakingProgressEvent;
...
public class AntiPhase
    ...
    private final BooleanSetting forecast = this.add(new BooleanSetting("Forecast", false).setParent());
    private final SliderSetting forecastPriority = this.add(new SliderSetting("ForecastPriority", 70, 0, 100, this.forecast::getValue));
    private final Timer timer = new Timer();
    private boolean forecastActive = false;
    private BlockPos forecastHeadPos = null;
    ...
    public void onBlockBreakingProgress(BlockBreakingProgressEvent event) {
        if (!this.forecast.getValue() || Module.nullCheck()) {
            return;
        }
        BlockPos feetPos = AntiPhase.mc.player.getBlockPos();
        if (!event.getPosition().equals(feetPos)) {
            return;
        }
        if (AntiPhase.mc.player.isCrawling()) {
            return;
        }
        if (event.getProgress() == 255) {
            if (this.forecastActive) {
                this.cancelForecast();
            }
        } else {
            if (!this.forecastActive) {
                BlockPos headPos = feetPos.up();
                this.forecastHeadPos = headPos;
                this.forecastActive = true;
                PacketMine.INSTANCE.mine(headPos);
            }
        }
    }
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (this.forecastActive && this.forecastHeadPos != null) {
            if (AntiPhase.mc.world.isAir(AntiPhase.mc.player.getBlockPos()) || AntiPhase.mc.world.isAir(this.forecastHeadPos)) {
                this.cancelForecast();
            }
        }
    ...
    private void cancelForecast() {
        if (this.forecastHeadPos != null) {
            Direction side = BlockUtil.getClickSide(this.forecastHeadPos);
            if (side != null) {
                sendSequencedPacket(id -> new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.STOP_DESTROY_BLOCK, this.forecastHeadPos, side, id));
            }
        }
        this.forecastActive = false;
        this.forecastHeadPos = null;
    }
}